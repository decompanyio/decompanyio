...
*/
  ...
  it("Setting up...", async () => {
    ...
    await _voteMap.init(_util.address);
  ...
  it("Create a Vote", async () => {
    ...
    const voteId = await _voteMap.next();
    //console.log("voteId : " + voteId);

    _startTime = await _util.getDateMillis();
    await _voteMap.create(voteId, ADDR, DOC, DEPOSIT);
    _endTime = await _util.getDateMillis();
    //console.log("created");

    const deposit_D1_S1 = web3.fromWei(DEPOSIT, "ether") * 1;
    const deposit_D1_S2 = web3.fromWei(await _voteMap.getVoteDeposit(voteId), "ether") * 1;
    // assert
  ...
  it("Read a Vote", async () => {
    ...
    let voteId = await _voteMap.count();
    //console.log("voteId : " + voteId);

    const address_D1_S1 = ADDR;
    const address_D1_S2 = await _voteMap.getVoteAddress(voteId);
    assert.equal(address_D1_S1, address_D1_S2, "wrong address");

    const date_D1_S0 = _startTime;
    //console.log("date_D1_S0 : " + date_D1_S0*1);
    const date_D1_S1 = await _voteMap.getVoteStartDate(voteId);
    ...
    const claimed_D1_S2 = web3.fromWei(await _voteMap.getVoteClaimed(voteId), "ether") * 1;
